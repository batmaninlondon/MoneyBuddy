<%-- 
    Document   : SanityCheck
    Created on : 16 Sep, 2016, 10:44:06 AM
    Author     : ADMIN
--%>
...
<head>
    ...
    <title>MoneyBuddy - Sanity Check - MoneyBuddy.com</title>
    <meta charset="utf-8">
    ...
    <link rel="stylesheet" href="https://invest.wealthify.com/assets/css/style.min.css?v=266243253">
    <link rel="stylesheet" href="https://invest.wealthify.com/assets/css/print.min.css" media="print">

</head>

<body class="body-sanity-check">
    <!-- Google Tag Manager -->
                        ...
                        <ul class="account-nav-dropdown">
                            <li>
                                ...
                                <div class="account-nav-dropdown__container">
                                    <ul>
                                        <li><a href="Logoff.jsp">Sign out <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
                                    </ul>
                                </div>
                            </li>
                        </ul>
...
<div class="heading--wide">
    <div class="grid">
        <h1 data-heading="h1">Let&#39;s check your investment style</h1>
    </div>
</div>
<!-- end of heading -->
...
<!-- content--wide -->
<div class="content-wide sanity-check">


    
<div id="sanity-check-wrapper">
    <s:set var="riskCategory"><%=request.getParameter("riskCategory")%></s:set>
    <s:set var="plan"><%=request.getParameter("plan")%></s:set>
    <s:set var="upfrontInvestment"><%=request.getParameter("upfrontInvestment")%></s:set>
    <s:set var="sip"><%=request.getParameter("sip")%></s:set>
    <s:set var="years"><%=request.getParameter("years")%></s:set>
    <s:set var="predictedValue"><%=request.getParameter("predictedValue")%></s:set>

    <s:form action="SanityCheckAction">
        <h2> <s:radio
            label="Will you need to withdraw the money you are investing in this Plan?"
            name="answer1"
            list="#{'1':'Within 1 year','2':'Within 2 year','3':'Within 3 year','4':'Within 4 year','5':'Within 5 year'}"
            value="1"
        />
        
        <s:radio
            label="In your opinion, which of the following best describes a &quot;high-risk&quot; investment?"
            name="answer2"
            list="#{'1':'Certain low return','2':'Likely losses','3':'Chance of high return with possible losses','4':'Certain high return'}"
            value="1"
        />
        
        <s:radio
            label="If markets fell and this Plan lost half of its value, how would your future financial plans change?"
            name="answer3"
            list="#{'1':'Not at all','2':'Moderately','3':'Dramatically'}"
            value="1"
        />
        
        <s:radio
            label="If this Plan lost 20% of its value over a 12-month period, how would you feel?"
            name="answer4"
            list="#{'1':'Unconcerned','2':'Disappointed','3':'Worried','4':'Panicked'}"
            value="1"
        />
        
        <s:radio
            label="What is your main consideration when choosing an investment?"
            name="answer5"
            list="#{'1':'What I could gain','2':'What I could loose'}"
            value="1"
        />
        
        <s:radio
            label="Excluding any mortgages, how would you describe your current level of debt?"
            name="answer6"
            list="#{'1':'None','2':'More Savings than debt','3':'Less Savings than debt'}"
            value="1"
        />
        
        <s:radio
            label="Aside from the money you intend to invest in this Plan, do you have emergency savings? (Generally, this should be enough to cover your living expenses for 3 months.)"
            name="answer7"
            list="#{'1':'Yes','2':'No'}"
            value="1"
        />
        ...
        <s:hidden name="riskCategory" value="%{#riskCategory}"/>
        <s:hidden name="upfrontInvestment" value="%{#upfrontInvestment}"/>
        <s:hidden name="sip" value="%{#sip}"/>
        <s:hidden name="years" value="%{#years}"/>
        <s:hidden name="plan" value="%{#plan}"/>
        <s:hidden name="predictedValue" value="%{#predictedValue}"/>
        <s:submit/>
    </s:form>
