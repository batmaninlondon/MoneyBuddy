

<!DOCTYPE html>
<!--[if lt IE 7]>   <html class="no-js lt-ie10 lt-ie9 lt-ie8"> <![endif]-->
<!--[if IE 8]>      <html class="no-js lt-ie10 lt-ie9 is-ie8"> <![endif]-->
<!--[if IE 9]>      <html class="no-js lt-ie10 is-ie9"> <![endif]-->
<!--[if gt IE 9]><!-->
<html class="no-js">
<!--<![endif]-->

<head>
    <title>MoneyBuddy - Personal Investment Plans | ISAs - MoneyBuddy.com </title>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="description" content="MoneyBuddy is an online investment service which lets you build personal investment plans and then manages them for you. Start investing from &#163;250">
	<meta name="keywords" content="Easy investing, low cost investing, passive investing, cheap investing, simple investing, diversified portfolios">
	<link rel="canonical" href="http://www.MoneyBuddy.com" />
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="shortcut icon" href="//invest.MoneyBuddy.com/assets/images/favicons/favicon.ico"/>
    <link rel="apple-touch-icon" href="//invest.MoneyBuddy.com/assets/images/favicons/apple-touch-icon.png">

	<link rel="stylesheet" href="//invest.MoneyBuddy.com/assets/css/style.min.css?v=15353232">
	<link rel="stylesheet" href="//invest.MoneyBuddy.com/assets/css/print.min.css" media="print">

        <!-- Start - Commented by Savita Wadhwani
	<meta name="google-site-verification" content="IaLuhSe70m7YnfpM1yEbCbsV_WBYCbyigvFziMvfG9s" />
        End - Commented by Savita Wadhwani -->

	<meta name="twitter:card" content="summary_large_image">
	<meta name="twitter:site" content="@MoneyBuddy_com">
	<meta name="twitter:creator" content="@richbtheo">
	<meta name="twitter:title" content="Expect more from your money, let MoneyBuddy invest it for you">
	<meta name="twitter:description" content="Don't settle for bank savings rates. MoneyBuddy builds you a personal investment plan and manages it for you. capital@risk">
	<meta name="twitter:image" content="https://invest.MoneyBuddy.com/assets/images/MoneyBuddy-twitter-card-large-image.png">

</head>


<body class="page--home">
	<!-- Google Tag Manager -->
        <!-- Start - Commented by Savita Wadhwani
	<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
	new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
	j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
	'//www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
	})(window,document,'script','dataLayer','GTM-5R35VM');</script>

        End - Commented by Savita Wadhwani -->
	<!-- End Google Tag Manager -->

	<svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
    <symbol id="add" viewBox="6 6 20.1 20.1">
      <path d="M26.1 16.1l-7.8-2.3L16.1 6l-2.3 7.8L6 16.1l7.8 2.2 2.3 7.8 2.2-7.8z"/>
    </symbol>
    <symbol id="arrow-back" viewBox="12.9 8.2 7.1 14.3">
      <path d="M20 22.5l-7.1-7.2L20 8.2l-4 7.1z"/>
    </symbol>
    <symbol id="arrow-dropdown" viewBox="4.7 10 22.3 11.1">
      <path d="M27 10L15.9 21.1 4.7 10l11.2 6.2z"/>
    </symbol>
    <symbol id="arrow-sliders" viewBox="0 0 32 32">
      <path d="M17.9 9l7.1 7.1-7.1 7.1 4-7.1-4-7.1zm-7.6 7.1l4-7.1-7.1 7.1 7.1 7.1-4-7.1z"/>
    </symbol>
    <symbol id="arrow-small" viewBox="12.9 8.2 7.1 14.3">
      <path d="M12.9 8.2l7.1 7.2-7.1 7.1 4-7.1z"/>
    </symbol>
    <symbol id="arrow" viewBox="12.9 8.2 7.1 14.3">
      <path d="M12.9 8.2l7.1 7.2-7.1 7.1 4-7.1z"/>
    </symbol>
    <symbol id="back-arrow" viewBox="12.9 8.2 7.1 14.3">
      <path d="M20 22.5l-7.1-7.2L20 8.2l-4 7.1z"/>
    </symbol>
    <symbol id="bin" viewBox="14.292 15 12.906 15">
      <path d="M26.793 16.362h-3.828v-.846c0-.234-.19-.424-.422-.424h-3.388c-.233 0-.424.19-.424.424v.846h-3.807c-.234 0-.424.19-.424.423v1.27c0 .233.19.423.423.423h.846v9.947c0 1.04.422 1.48 1.586 1.48h7.196c1.006 0 1.375-.664 1.375-1.48v-9.947h.848c.234 0 .424-.19.424-.423v-1.27c0-.233-.173-.423-.407-.423zm-1.71 12.064c.013.634-.833.634-.833.634h-6.787s-.847.02-.847-.634V18.48h8.468v9.946zM20.85 27.122c-.19 0-.424-.092-.424-.203v-6.6c0-.112.235-.203.424-.203s.424.09.424.204v6.598c0 .112-.235.204-.424.204zM18.31 27.122c-.22 0-.424-.092-.424-.203v-6.6c0-.112.204-.203.424-.203s.422.09.422.204v6.598c0 .112-.203.204-.422.204zM23.39 27.122c-.18 0-.425-.092-.425-.203v-6.6c0-.112.246-.203.424-.203s.42.09.42.204v6.598c0 .112-.244.204-.42.204z"/>
    </symbol>
    <symbol id="checkmark" viewBox="0 0 32 32">
      <path stroke="#449FDB" d="M0 0"/><path d="M28.998 8.53l-2.134-2.133c-.394-.393-1.03-.393-1.423 0L12.647 19.192l-6.086-6.13c-.393-.393-1.03-.393-1.423 0l-2.134 2.134c-.393.394-.393 1.03 0 1.423l8.924 8.983c.393.393 1.03.393 1.423 0l15.648-15.65c.393-.39.393-1.03 0-1.422z"/>
    </symbol>
    <symbol id="close" viewBox="0 0 11.3 11.3">
      <path fill-rule="evenodd" clip-rule="evenodd" d="M1.4 0l9.9 9.9-1.4 1.4L0 1.4 1.4 0zM0 9.9L9.9 0l1.4 1.4-9.9 9.9L0 9.9z"/>
    </symbol>
    <symbol id="create-plan" viewBox="587 1286.2 18.1 17.2">
      <path d="M604 1299.7h-3.5c-.4-1-1.4-1.7-2.5-1.7-1.2 0-2.1.7-2.5 1.7H588c-.6 0-1 .4-1 1 0 .5.4 1 1 1h7.4c.4 1 1.4 1.7 2.5 1.7 1.2 0 2.1-.7 2.5-1.7h3.5c.6 0 1-.4 1-1 .1-.6-.3-1-.9-1zm-6 1.7c-.4 0-.7-.3-.7-.7 0-.4.3-.7.7-.7s.7.3.7.7-.3.7-.7.7zM588 1289.9h9.4c.4 1 1.4 1.7 2.5 1.7 1.2 0 2.1-.7 2.5-1.7h1.5c.6 0 1-.4 1-1 0-.5-.4-1-1-1h-1.5c-.4-1-1.4-1.7-2.5-1.7-1.2 0-2.1.7-2.5 1.7H588c-.6 0-1 .4-1 1s.4 1 1 1zm12-1.7c.4 0 .7.3.7.7 0 .4-.3.7-.7.7s-.7-.3-.7-.7c-.1-.3.3-.7.7-.7zM604 1293.8h-9.5c-.4-.9-1.4-1.6-2.5-1.6s-2.1.7-2.5 1.6H588c-.6 0-1 .4-1 1 0 .5.4 1 1 1h1.5c.4 1 1.4 1.8 2.6 1.8 1.2 0 2.2-.8 2.6-1.8h9.4c.6 0 1-.4 1-1-.1-.5-.5-1-1.1-1zm-12 1.8c-.4 0-.7-.3-.7-.7 0-.4.3-.7.7-.7s.7.3.7.7c.1.4-.3.7-.7.7z"/>
    </symbol>
    <symbol id="decline" viewBox="812 911 20 20">
      <path d="M822 931c5.5 0 10-4.5 10-10s-4.5-10-10-10-10 4.5-10 10 4.5 10 10 10zm-6.1-10.7c.4-.7 1.4-.9 2-.3.7.6 1.4 1.3 2 2 .2.2.4.4.6.7h.1v-6.2c0-.6.4-1.1.9-1.3s1.1-.1 1.5.4c.3.3.4.7.4 1.1V922.8l.2-.2c.8-.8 1.5-1.6 2.3-2.3.5-.5 1-.6 1.6-.3.5.2.8.7.8 1.3 0 .3-.1.7-.4.9l-5.1 5.1c-.5.5-1.3.5-1.8 0-.2-.2-.4-.3-.5-.5l-4.4-4.4c-.5-.8-.6-1.5-.2-2.1z"/>
    </symbol>
    <symbol id="double-arrow-small" viewBox="12.9 8.2 16.2 14.3">
      <path d="M22 8.2l7.1 7.2-7.1 7.1 4-7.1z"/><path d="M12.9 8.2l7.1 7.2-7.1 7.1 4-7.1z"/>
    </symbol>
    <symbol id="double-back-arrow" viewBox="12.9 8.2 16.2 14.3">
      <path d="M20 22.5l-7.1-7.2L20 8.2l-4 7.1z"/><path d="M29.1 22.5L22 15.3l7.1-7.1-4 7.1z"/>
    </symbol>
    <symbol id="download" viewBox="0 0 16 17">
      <path d="M6.5 7.9V1.5c0-.6.4-1.2 1-1.4C8-.1 8.7 0 9.1.5c.3.3.4.7.4 1.1V8c.1-.1.2-.2.3-.2.8-.8 1.7-1.6 2.5-2.4.5-.5 1.1-.6 1.7-.3.6.2.9.8.9 1.4 0 .4-.1.7-.4.9-1.8 1.6-3.7 3.3-5.5 5.1-.5.5-1.4.5-2 0-.2-.2-.4-.4-.6-.5-1.6-1.5-3.2-3.1-4.8-4.6-.5-.6-.7-1.3-.2-1.9.5-.7 1.5-.9 2.2-.3.7.6 1.4 1.3 2.1 2 .3.2.5.5.8.7-.1 0 0 0 0 0zM8 17H1.5c-.7 0-1.4-.5-1.5-1.2-.1-.8.4-1.4 1.1-1.6h13.2c.8 0 1.4.4 1.6 1.1.3.9-.4 1.8-1.4 1.8-2.2-.1-4.3-.1-6.5-.1z"/>
    </symbol>
    <symbol id="edit" viewBox="8.7 8.1 15.3 15.4">
      <path d="M24 11.5c-.1.9-.4 1.6-1.1 2.3l-8.1 8.1c-.4.5-.9.7-1.5.8-1.2.2-2.4.5-3.6.7-.6.1-1.1-.3-1-1 .3-1.5.6-3 .9-4.4 0-.2.2-.4.3-.5l8.4-8.4c1.9-1.9 4.9-1.1 5.6 1.5.1.3.1.6.1.9zm-9.8 8.6l.3-.3c1.2-1.2 2.5-2.4 3.7-3.7l3.6-3.6c.9-.9.7-2.3-.5-2.7-.7-.3-1.4-.1-1.9.5l-7.3 7.3-.2.2c1.1.5 1.9 1.2 2.3 2.3zm-1 .8c-.2-1.1-1-2-2.1-2.1-.1.5-.2 1-.3 1.4l.2-.2c.3.3.6.7 1 1.1.3 0 .7-.1 1.2-.2z"/>
    </symbol>
    <symbol id="facebook" viewBox="10 1.7 13 30.3">
      <path d="M20.3 7.6H23V1.7h-4.3c-4.6 0-5.8 3.9-5.8 6.4v3.7H10v4.3h2.9V32h5.8V16.1h4.2l.1-4.3h-4.3V8.4c0-1.2 1.1-.8 1.6-.8z"/>
    </symbol>
    <symbol id="googleplus" viewBox="34 33.5 32 32.9">
      <path d="M54.8 33.5H47c-3.5 0-5.9.8-8.1 2.6-1.7 1.5-2.7 3.6-2.7 5.7 0 3.3 2.5 6.7 7.1 6.7.4 0 .9 0 1.4-.1l-.1.2c-.2.4-.4.9-.4 1.5 0 1.3.6 2.1 1.2 2.8l.1.1h-.1c-1.9.1-5.5.4-8 2-3.1 1.8-3.3 4.5-3.3 5.2 0 3.1 2.8 6.1 9.2 6.1 7.4 0 11.3-4.1 11.3-8.1 0-3-1.7-4.5-3.6-6L49.4 51c-.5-.4-1.1-.9-1.1-1.8s.6-1.5 1.1-2l.1-.1c1.7-1.3 3.6-2.8 3.6-6.1s-2.1-5-3-5.8h2.7l2.2-1.4c.1 0 .1-.1.1-.2-.1 0-.2-.1-.3-.1zm-9.5 31.2c-4.5 0-7.5-2.1-7.5-5.2 0-2 1.2-3.5 3.7-4.4 2-.7 4.5-.7 4.5-.7h1c3.2 2.2 4.7 3.4 4.7 5.7-.1 2.8-2.5 4.6-6.4 4.6zm0-17.3c-3.8 0-5.4-5-5.4-7.6 0-1.4.3-2.4.9-3.2.7-.9 1.9-1.4 3-1.4 3.5 0 5.4 4.7 5.4 7.9 0 .5 0 2.1-1.1 3.1-.6.7-1.8 1.2-2.8 1.2zM65.8 48.9h-4.1v-4.1c0-.1-.1-.2-.2-.2h-1.8c-.1 0-.2.1-.2.2v4.1h-4.1c-.1 0-.2.1-.2.2v1.8c0 .1.1.2.2.2h4.1v4.1c0 .1.1.2.2.2h1.8c.1 0 .2-.1.2-.2v-4.1h4.1c.1 0 .2-.1.2-.2v-1.8c0-.1-.1-.2-.2-.2z"/>
    </symbol>
    <symbol id="graph" viewBox="237 312 38 34">
      <path d="M240 336h2c1.7 0 3 1.3 3 3v4c0 1.7-1.3 3-3 3h-2c-1.7 0-3-1.3-3-3v-4c0-1.7 1.3-3 3-3zM250 328h2c1.7 0 3 1.3 3 3v12c0 1.7-1.3 3-3 3h-2c-1.7 0-3-1.3-3-3v-12c0-1.7 1.3-3 3-3zM260 320h2c1.7 0 3 1.3 3 3v20c0 1.7-1.3 3-3 3h-2c-1.7 0-3-1.3-3-3v-20c0-1.7 1.3-3 3-3zM270 312h2c1.7 0 3 1.3 3 3v28c0 1.7-1.3 3-3 3h-2c-1.7 0-3-1.3-3-3v-28c0-1.7 1.3-3 3-3z"/>
    </symbol>
    <symbol id="growth" viewBox="812 911 20 20">
      <path d="M822 911c-5.5 0-10 4.5-10 10s4.5 10 10 10 10-4.5 10-10-4.5-10-10-10zm6.1 10.7c-.4.7-1.4.9-2 .3-.7-.6-1.4-1.3-2-2-.2-.2-.4-.4-.6-.7h-.1v6.2c0 .6-.4 1.1-.9 1.3s-1.1.1-1.5-.4c-.3-.3-.4-.7-.4-1.1V919.2l-.2.2c-.8.8-1.5 1.6-2.3 2.3-.5.5-1 .6-1.6.3-.5-.2-.8-.7-.8-1.3 0-.3.1-.7.4-.9l5.1-5.1c.5-.5 1.3-.5 1.8 0 .2.2.4.3.5.5l4.4 4.4c.5.8.6 1.5.2 2.1z"/>
    </symbol>
    <symbol id="hamburger" viewBox="0 0 14 12">
      <path fill-rule="evenodd" clip-rule="evenodd" d="M0 0h14v2H0V0zM0 10h14v2H0v-2zM0 5h14v2H0V5z"/>
    </symbol>
    <symbol id="info" viewBox="12.7 8.2 6.8 15.4">
      <path d="M19.3 21.9l-.2.9c-.6.3-1.2.4-1.5.6-.4.1-.8.2-1.3.2-.8 0-1.4-.2-1.8-.6s-.6-.9-.6-1.5v-.7s.1-.5.2-.8l.8-2.9c.1-.3.1-.5.2-.8s-.1-.3-.1-.5c0-.4-.1-.6-.2-.8-.2-.1-.4-.2-.9-.2-.2 0-.4 0-.6.1-.3.1-.4.1-.6.2l.2-.9 1.5-.6s.9-.2 1.4-.2c.8 0 1.4.2 1.8.6.4.4.6.9.6 1.5v.7s-.1.5-.2.8l-.8 2.8c-.1.2-.1.5-.2.8v.6c0 .4.1.6.3.8s.5.2.9.2c.2 0 .4 0 .7-.1s.3-.1.4-.2zm.2-11.9c0 .5-.2.9-.6 1.3s-.8.5-1.4.5-1-.2-1.4-.5-.6-.8-.6-1.3.2-.9.6-1.3c.4-.4.8-.5 1.4-.5s1 .2 1.4.5.6.8.6 1.3z"/>
    </symbol>
    <symbol id="instagram" viewBox="0 0 32 32">
      <path d="M25.9 3h-19C5.2 3 4 4.5 4 5.8v20.3C4 27.8 5.2 29 6.9 29h18.8c1.7 0 3.2-1.3 3.2-3V6c0-1.7-1.3-3-3-3zm-6 3.6c0-.4.3-.7.7-.7H25c.4 0 .7.3.7.7v4.3c0 .4-.3.7-.7.7h-4.3c-.4 0-.7-.3-.7-.7V6.6zm-3.1 4.7c2.8 0 5 2.2 5 5 0 2.7-2.2 5-5 5s-5-2.2-5-5 2.2-5 5-5zm9.3 14.8H7.5c-.4 0-.6-.2-.6-.6v-11h2.3c-.2 1.5-.3 1.2-.3 1.9 0 4.3 3.5 7.8 7.9 7.8 4.3 0 7.9-3.4 7.9-7.7 0-.7-.1-.5-.3-2h1.3v10.9c0 .5.8.7.4.7z"/>
    </symbol>
    <symbol id="less" viewBox="9 15 13.9 2">
      <path d="M22.9 16.9L9 17v-2h13.9z"/>
    </symbol>
    <symbol id="linkedin" viewBox="199.1 199.8 32 30.6">
      <path d="M231.1 218.5v11.8h-6.9v-11c0-2.8-1-4.7-3.5-4.7-1.9 0-3 1.3-3.5 2.5-.2.4-.2 1.1-.2 1.7v11.5h-6.9s.1-18.7 0-20.6h6.9v3-.1c.9-1.4 2.5-3.4 6.2-3.4 4.5 0 7.9 3 7.9 9.3zm-28.2-18.7c-2.3 0-3.9 1.5-3.9 3.6 0 2 1.5 3.6 3.8 3.6 2.4 0 3.9-1.6 3.9-3.6 0-2.1-1.4-3.6-3.8-3.6zm-3.4 30.5h6.9v-20.6h-6.9v20.6z"/>
    </symbol>
    <symbol id="livechat" viewBox="0 0 20 21">

      <!--<defs><style>.cls-1 { fill: #fff; } .cls-1, .cls-2 { fill-rule: evenodd; } .cls-2 { stroke: #fff; stroke-width: 1px; fill: none; }</style></defs><path d="M15.266 11.53c-.872 0-1.578-.698-1.578-1.56 0-.864.706-1.564 1.578-1.564.87 0 1.578.7 1.578 1.563 0 .862-.707 1.56-1.578 1.56zm-5.266 0c-.863 0-1.563-.698-1.563-1.56 0-.864.7-1.564 1.563-1.564s1.563.7 1.563 1.563c0 .862-.7 1.56-1.563 1.56zm-5.266 0c-.87 0-1.578-.698-1.578-1.56 0-.864.707-1.564 1.578-1.564.872 0 1.578.7 1.578 1.563 0 .862-.706 1.56-1.578 1.56z" class="cls-1"/><path d="M3.158 0h13.684C18.586 0 20 1.41 20 3.15v9.45c0 1.74-1.256 3.4-3 3.4H8s-5.944 5-5.895 5C.248 21 0 19.95 0 19.95V3.15C0 1.41 1.414 0 3.158 0z" class="cls-2"/>-->
    </symbol>
    <symbol id="menu" viewBox="0 0 32 28">
      <path d="M0 0h32v4H0zM0 12h32v4H0zM0 24h32v4H0z"/>
    </symbol>
    <symbol id="nb" viewBox="0 0 32 32">
      <path d="M26 2c.5 0 1 .5 1 1v21c0 .5-.5 1-1 1h-6v2.3l-2.9-1.9-.5-.4H5c-.5 0-1-.5-1-1V3c0-.5.5-1 1-1h21m0-2H5C3.4 0 2 1.4 2 3v21c0 1.6 1.4 3 3 3h11l6 4v-4h4c1.6 0 3-1.4 3-3V3c0-1.6-1.4-3-3-3zM9 11.6l-.8-1.4h-.1l.1 1.4V18h-1V8h.6l4.7 6.5.7 1.3h.1l-.2-1.3V8.2h1.1v10h-.6L9 11.6zm7.5-3.4h1.1v3.5h.1c.2-.3.5-.5.8-.6.3-.1.7-.2 1.1-.2.9 0 1.6.3 2 .9.5.6.7 1.5.7 2.7 0 1.2-.3 2.2-.9 2.8-.6.6-1.4 1-2.5 1-.5 0-1-.1-1.4-.2s-.7-.2-1-.4V8.2zm2.9 3.6c-.5 0-.8.1-1.1.4s-.5.6-.6 1.1v3.5c.2.1.4.2.6.2.3.1.5.1.8.1.6 0 1.1-.2 1.5-.7.4-.4.6-1.1.6-2.1 0-.4 0-.7-.1-1-.1-.3-.2-.6-.3-.8-.1-.2-.3-.4-.5-.5-.3-.1-.6-.2-.9-.2zm4.1 5.6c0-.3.1-.4.2-.6.1-.1.3-.2.6-.2s.5.1.6.2c.1.1.2.3.2.6 0 .2-.1.4-.2.6-.1.2-.3.2-.6.2-.2 0-.4-.1-.6-.2-.1-.2-.2-.4-.2-.6z"/>
    </symbol>
    <symbol id="paper" viewBox="960 965 24 32">
      <path d="M981.3 971h-2.5c-.4 0-.8-.4-.8-.9v-2.2c0-1.6-1.2-2.9-2.8-2.9h-12.5c-1.5 0-2.7 1.3-2.7 2.9v26.2c0 1.6 1.2 2.9 2.7 2.9h18.5c1.5 0 2.8-1.3 2.8-2.9V973.9c0-1.6-1.2-2.9-2.7-2.9zm.7 23.1c0 .5-.3.9-.8.9h-18.5c-.4 0-.8-.4-.8-.9v-26.2c0-.5.3-.9.8-.9H975.3c.4 0 .8.4.8.9v2.2c0 1.6 1.2 2.9 2.8 2.9h2.5c.4 0 .8.4.8.9V994.1h-.2zM965 975h8c.6 0 1-.4 1-1s-.4-1-1-1h-8c-.6 0-1 .4-1 1s.4 1 1 1zM979 977h-14c-.6 0-1 .4-1 1s.4 1 1 1h14c.6 0 1-.4 1-1s-.4-1-1-1zM979 981h-14c-.6 0-1 .4-1 1s.4 1 1 1h14c.6 0 1-.4 1-1s-.4-1-1-1zM979 985h-14c-.6 0-1 .4-1 1s.4 1 1 1h14c.6 0 1-.4 1-1s-.4-1-1-1zM979 989h-14c-.6 0-1 .4-1 1s.4 1 1 1h14c.6 0 1-.4 1-1s-.4-1-1-1z"/>
    </symbol>
    <symbol id="phone" viewBox="0 0 20 20">
      <path d="M2.75 1.156C1.156 2.344.875 3.876.844 4.813.656 9.625 6.75 18.25 15.03 19.22c2.25-.157 3.75-1.72 3.876-2.126.34-.625.33-1.25.327-1.688.02-.813-.312-.632-1.87-1.63s-2.207-1.62-2.904-1.178c-.5 0-1.71 2.495-2.647 2.214-2.032-.937-5.22-3.312-6.687-6.896 0-.916 1.68-1.426 2.306-2.395.412-.645-.62-1.52-1.06-2.553C5.916 2.234 5.374.703 4.707.703 4.425.698 3.42.858 2.75 1.156z"/>
    </symbol>
    <symbol id="print" viewBox="0 0 24.998 24.998">
      <path d="M24.526 7.62H.51c-.204 0-.37.165-.37.37v9.303c0 .204.167.37.37.37H3.7v-4.2c0-.51.415-.924.924-.924h15.79c.508 0 .923.413.923.923v4.2h3.19c.203 0 .37-.166.37-.37V7.988c0-.203-.167-.368-.37-.368zM21.336 5.63h-1.11V1.11H4.81v4.52H3.7V.925c0-.51.415-.924.924-.924h15.79c.508 0 .923.414.923.924V5.63zM3.7 17.35v-4.2c0-.51.415-.924.924-.924h15.79c.508 0 .923.414.923.924V23.9c0 .51-.415.924-.924.924H4.623c-.508 0-.923-.414-.923-.924v-6.55m1.11 6.365h15.418V13.333H4.808v10.382zM6.44 14.858h8.49v1.108H6.44zM6.44 17.99h12.155v1.11H6.44zM6.44 21.122h12.155v1.108H6.44z"/><path fill="#fff" d="M23.492 11.057c0 .204-.166.37-.37.37h-1.97c-.205 0-.37-.166-.37-.37V9.5c0-.206.165-.37.37-.37h1.97c.204 0 .37.164.37.37v1.557z"/>
    </symbol>
    <symbol id="refresh" viewBox="0 0 15 12.516">
      <g fill="#010202"><path d="M4.145.99C4.937.44 5.967.068 7.02.012c3.986-.208 6.68 2.302 6.533 5.744 0 0 .47-.504.656-.756.112-.126.364-.107.528-.036.108.043.252.186.262.347-.008.096-.03.164-.107.248l-1.4 1.524c-.113.127-.447.334-.723.034-.078-.062-.94-.946-1.365-1.363-.17-.152-.22-.35-.11-.517.107-.207.535-.34.705-.188.21.15.662.708.662.708-.027-3.03-2.346-4.805-5.294-4.71-.92.03-1.958.325-2.64.792-.305.167-.573-.025-.697-.21-.13-.19-.04-.528.115-.638zM10.842 11.538c-.795.55-1.826.917-2.88.968-3.985.197-6.672-2.323-6.512-5.764 0 0-.472.502-.66.754-.113.128-.365.106-.53.035C.155 7.488.01 7.344 0 7.184c.01-.096.032-.162.107-.247l1.407-1.52c.114-.127.447-.333.724-.032.078.062.935.95 1.36 1.368.168.152.22.35.108.518-.11.206-.537.337-.706.186-.21-.15-.66-.71-.66-.71.018 3.03 2.33 4.812 5.28 4.727.92-.026 1.958-.32 2.642-.784.305-.166.572.025.695.21.13.19.04.528-.115.638z"/></g>
    </symbol>
    <symbol id="save" viewBox="9 10 13 13">
      <path d="M9 10v10l3 3h10V10H9zm1 3v-2h2v2h-2zm10 9h-4v-3h-2v3h-2v-5h8v5z"/>
    </symbol>
    <symbol id="send" viewBox="0 0 70.87 70.87">
      <path d="M20.088 46.35c-.03-.328.102-.65.357-.863l28.68-23.818-35.507 18.64c-.305.16-.67.146-.964-.025l7.273 16.4 6.055-6.57c-2.926-1.633-5.324-2.95-5.38-2.982-.29-.16-.482-.452-.514-.782zM38.877 57.06c-.748 0-1.564-.312-2.497-.95-2.252-1.54-15.643-8.903-15.778-8.978-.29-.16-.482-.45-.514-.78-.03-.33.102-.653.357-.865l28.68-23.818-35.507 18.64c-.32.167-.706.15-1.007-.046L1.997 33.407c-.968-.58-1.44-1.278-1.374-2.06.097-1.14 1.312-1.667 1.554-1.762.67-.243 64.78-23.462 65.644-23.793.504-.193.94-.288 1.335-.288.88 0 1.303.47 1.476.752.247.398.43 1.092-.093 2.137-.863 1.613-28.325 45.293-29.795 47.632-.228.418-.832 1.034-1.865 1.034zM22.85 46.087c3.46 1.91 12.73 7.05 14.66 8.37.79.542 1.22.6 1.367.6.085 0 .123-.018.13-.023 9.783-15.57 28.217-44.933 29.678-47.43-.046.016-.096.033-.148.054-.875.335-63.014 22.84-65.66 23.798-.03.013-.07.034-.112.058.068.054.162.12.287.193l10.153 6.557L56.61 15.48c.453-.24 1.014-.096 1.298.33.284.425.2.997-.193 1.324L22.85 46.088z"/>
    </symbol>
    <symbol id="share" viewBox="0 0 20 20">
      <circle cx="16.425" cy="3.577" r="3.427"/><circle cx="16.425" cy="16.572" r="3.427"/><circle cx="3.507" cy="10.305" r="3.427"/><path d="M16.097 2.948L16.75 4.2 3.834 10.935 3.18 9.683zM3.815 9.67l12.92 6.265-.617 1.272-12.92-6.267z"/>
    </symbol>
    <symbol id="shield" viewBox="138.1 796 10.9 11">
      <path d="M149 796.4v4.9c0 .8-.2 1.6-.6 2.3-.9 1.7-2.6 2.8-4.8 3.4h-.2c-2.9-.9-4.7-2.5-5.3-4.9v-5.7c3.6-.5 7.2-.5 10.9 0z"/>
    </symbol>
    <symbol id="speech-dialog" viewBox="48 48 32 32">
      <path d="M77.1 55h-4.3v-4.3c0-1.5-1.3-2.8-2.9-2.8h-19c-1.6 0-2.9 1.2-2.9 2.8V70c0 .8.3 1.6.9 2.1.6.6 1.3.9 2.1.9s1.5-.3 2.1-.9l2.2-2.1c.3 1.2 1.5 2 2.8 2h8.5c.7 0 1.3.3 1.8.8l6.6 6.3c.6.6 1.3.9 2.1.9s1.6-.3 2.1-.9.8-1.3.8-2.1V57.8c0-1.5-1.3-2.8-2.9-2.8zM51.7 70.7c-.4.4-1 .4-1.4 0-.2-.2-.3-.4-.3-.7V50.8c0-.4.4-.8.9-.8H70c.5 0 .9.3.9.8v11.5c0 .4-.4.8-.9.8h-8.5c-1.2 0-2.3.5-3.2 1.3l-6.6 6.3zM78 77c0 .3-.1.5-.3.7-.4.4-1 .4-1.4 0l-6.6-6.3c-.8-.8-2-1.3-3.2-1.3H58c-.5 0-.9-.3-.9-.8v-1.1l2.6-2.5c.5-.5 1.1-.7 1.8-.7H70c1.6 0 2.9-1.2 2.9-2.8V57h4.3c.5 0 .9.3.9.8V77zM60.5 60c.8 0 1.5-.7 1.5-1.5s-.7-1.5-1.5-1.5-1.5.7-1.5 1.5.7 1.5 1.5 1.5zM54.5 57c-.8 0-1.5.7-1.5 1.5s.7 1.5 1.5 1.5 1.5-.7 1.5-1.5-.7-1.5-1.5-1.5zM66.5 60c.8 0 1.5-.7 1.5-1.5s-.7-1.5-1.5-1.5-1.5.7-1.5 1.5.7 1.5 1.5 1.5z"/>
    </symbol>
    <symbol id="stats" viewBox="862 309.9 44 36.2">
      <path d="M905.7 312c.4-.5.4-1.3-.2-1.8-.5-.4-1.3-.4-1.8.2l-5.4 6.5c-.2 0-.4-.1-.6-.1-2.1-.2-3.9 1.3-4.1 3.4 0 .4 0 .8.1 1.1l-5.1 5.2c-.5-.4-1.2-.7-1.9-.8-2-.2-3.8 1.2-4.1 3.2l-7.5 3.4c-.6-.9-1.6-1.6-2.8-1.7-2.1-.2-3.9 1.3-4.1 3.4-.1.8.1 1.6.5 2.2l-6.4 7.8c-.4.5-.4 1.3.2 1.8.2.2.4.2.7.3.4 0 .8-.1 1.1-.5l6.3-7.7c.3.1.6.2 1 .2 2 .2 3.8-1.2 4.1-3.2l7.5-3.4c.6.9 1.6 1.6 2.8 1.7 2.1.2 3.9-1.3 4.1-3.4 0-.4 0-.8-.1-1.1l5.1-5.2c.5.4 1.2.7 1.9.8 2.1.2 3.9-1.3 4.1-3.4.1-1-.2-1.8-.7-2.6l5.3-6.3zm-33.9 23.6c-.7-.1-1.2-.7-1.1-1.4.1-.7.7-1.2 1.4-1.1s1.2.7 1.1 1.4c-.1.6-.7 1.1-1.4 1.1zm14.5-4.9c-.7-.1-1.2-.7-1.1-1.4.1-.7.7-1.2 1.4-1.1s1.2.7 1.1 1.4-.7 1.2-1.4 1.1zm11-8.9c-.7-.1-1.2-.7-1.1-1.4.1-.7.7-1.2 1.4-1.1s1.2.7 1.1 1.4c-.1.7-.7 1.2-1.4 1.1z"/>
    </symbol>
    <symbol id="stopwatch" viewBox="0 0 20 20">
      <path d="M16.81 6.627s.953-.967 1.34-1.354c.627-.627 0-1.228 0-1.228l-1.172-1.172s-.626-.506-1.15.02c-.364.363-1.472 1.49-1.472 1.49-.674-.41-1.41-.725-2.188-.93v-2.35c0-1.11-1.088-1.087-1.088-1.087H9.008S8.006.12 8.006 1.05v2.365c-1.515.372-2.87 1.156-3.94 2.226-1.524 1.524-2.468 3.63-2.468 5.95 0 2.318.944 4.423 2.468 5.946s3.627 2.468 5.947 2.467c2.32 0 4.425-.942 5.95-2.467s2.466-3.628 2.466-5.947c0-1.855-.604-3.57-1.62-4.963zm-1.598 10.16c-1.332 1.333-3.17 2.158-5.2 2.157-2.028 0-3.867-.825-5.2-2.157s-2.156-3.17-2.156-5.198.825-3.87 2.158-5.2c1.332-1.334 3.17-2.158 5.2-2.158s3.866.825 5.198 2.157c1.332 1.33 2.157 3.17 2.156 5.2 0 2.027-.824 3.866-2.156 5.198zM10.96 10.92s.227.42.227.68c0 .655-.532 1.188-1.187 1.188s-1.187-.532-1.187-1.187c0-.277.096-.532.256-.734L10.01 5.5l.947 5.42z"/>
    </symbol>
    <symbol id="tick" viewBox="714.8 1205.6 16.3 12.7">
      <path d="M721.3 1215.3c1.1-1.3 2.1-2.5 3.2-3.8 1.5-1.8 3-3.5 4.4-5.3.3-.4.7-.7 1.3-.5.9.2 1.2 1.2.6 2-.9 1.1-1.8 2.1-2.6 3.2-1.9 2.3-3.8 4.6-5.8 6.9-.6.7-1.4.8-2 .1-1.7-1.9-3.5-3.8-5.2-5.7-.5-.6-.5-1.3 0-1.7.5-.4 1.2-.4 1.7.1 1.4 1.5 2.8 3 4.1 4.5.2 0 .2.1.3.2z"/>
    </symbol>
    <symbol id="transfer-in" viewBox="440 520 19.9 18.1">
      <path d="M441 525.2h1c.6 0 1.2.1 1.7.4.1.1.3.1.4.1.4 0 .7-.2.9-.5.2-.5 0-1-.4-1.3-.8-.4-1.7-.6-2.6-.6h-1c-.5 0-1 .4-1 .9 0 .6.4 1 1 1zM456.1 529.6c-.2-.2-.4-.2-.6-.3-.4 0-.8.2-.9.6-.2.4-.1.8.2 1.1.6.5 1.1 1.1 1.7 1.6l.2.2H451.7c-.6 0-1.2-.1-1.7-.4-.5-.2-1.1 0-1.3.4-.2.5 0 1 .4 1.3.8.4 1.7.6 2.6.6H456.5c-.2.1-.3.3-.5.4-.5.5-1 .9-1.4 1.4-.4.4-.3 1.1.2 1.4.4.3.9.2 1.3-.2 1.1-1 2.1-2 3.2-3 .1-.1.3-.2.4-.4.3-.4.3-.9 0-1.2-1.2-1.2-2.4-2.4-3.6-3.5zM451.7 525.3h4.8c-.2.1-.3.3-.5.4-.5.5-1 .9-1.4 1.4-.4.4-.3 1.1.2 1.4.4.3.9.2 1.3-.2 1.1-1 2.1-2 3.2-3 .1-.1.3-.2.4-.4.3-.4.3-.9 0-1.2-1.2-1.1-2.4-2.3-3.6-3.4-.2-.2-.4-.2-.6-.3-.4 0-.8.2-.9.6-.2.4-.1.8.2 1.1.6.5 1.1 1.1 1.7 1.6l.2.2H451.8c-3.2 0-5.9 2.5-5.9 5.7 0 2.1-1.7 3.8-3.9 3.8h-1c-.5 0-1 .4-1 .9s.4.9 1 .9h1c3.2 0 5.8-2.5 5.8-5.7 0-2.1 1.8-3.8 3.9-3.8z"/>
    </symbol>
    <symbol id="twitter" viewBox="2 3.3 29 23.7">
      <path d="M31 6.1c-1.1.5-2.2.8-3.4.9 1.2-.7 2.2-1.9 2.6-3.3-1.2.8-2.4 1.3-3.8 1.5C25.3 4 23.8 3.3 22 3.3c-3.3 0-6 2.7-6 6 0 .5.1.9.2 1.4-5-.2-9.4-2.6-12.3-6.2-.5.9-.8 1.9-.8 3 0 2.1 1.1 3.9 2.7 5-1 0-1.9-.3-2.7-.7v.1c0 2.9 2.1 5.3 4.8 5.9-.5.1-1 .2-1.6.2-.4 0-.8 0-1.1-.1.8 2.4 3 4.1 5.6 4.1-2 1.6-4.6 2.6-7.4 2.6-.5 0-1 0-1.4-.1C4.5 26 7.7 27 11 27c11 0 17-9.1 17-17v-.8c1.2-.8 2.2-1.9 3-3.1z"/>
    </symbol>
    <symbol id="user" viewBox="0 0 28 28">
      <path d="M12.7 19.3c-3.6 0-6.6-3.6-6.6-8 0-2.1.7-4.1 1.9-5.6 1.2-1.6 2.9-2.4 4.7-2.4 3.6 0 6.6 3.6 6.6 8-.1 4.4-3 8-6.6 8zm0-14.3C10 5 7.9 7.8 7.9 11.3c0 3.4 2.2 6.2 4.8 6.2 2.7 0 4.8-2.8 4.8-6.2 0-3.5-2.2-6.3-4.8-6.3zM20.2 28H2.4C1.1 28 0 26.9 0 25.6c0-4 3.2-7.2 7.2-7.2.5 0 .9.4.9.9s-.4.9-.9.9c-3 0-5.4 2.4-5.4 5.4 0 .4.3.7.7.7H23c.4 0 .7-.3.7-.7 0-3-2.4-5.4-5.4-5.4-.5 0-.9-.4-.9-.9s.4-.9.9-.9c4 0 7.2 3.2 7.2 7.2 0 1.4-1.1 2.4-2.5 2.4h-2.8zM20.8 10.9h-.2c-.5-.1-.8-.6-.7-1.1.1-.6.2-1.2.2-1.8 0-3.4-2.2-6.2-4.8-6.2-.7 0-1.4.2-2 .6-.4.2-1 .1-1.2-.3-.2-.4-.1-1 .3-1.2.9-.6 1.9-.9 2.9-.9 3.6 0 6.6 3.6 6.6 8 0 .8-.1 1.5-.3 2.2-.1.4-.4.7-.8.7zM27 23.6c-.1 0-.2 0-.4-.1-.4-.2-.6-.7-.4-1.2 0-.1.1-.2.1-.3 0-3-2.3-5.5-5.2-5.5-.5 0-.9-.4-.9-.9s.4-.9.9-.9c4 0 7 3.3 7 7.2 0 .3-.1.7-.2 1-.3.5-.6.7-.9.7z"/>
    </symbol>
    <symbol id="withdraw" viewBox="315 519 16 17.1">
      <path d="M324.5 524.1v6.5c0 .6-.4 1.2-1 1.4s-1.3.1-1.7-.4c-.3-.3-.4-.7-.4-1.1v-6.4c-.1.1-.2.2-.3.2-.9.8-1.7 1.6-2.6 2.4-.5.5-1.1.6-1.7.3-.6-.2-.9-.8-.9-1.4 0-.4.1-.7.4-.9 1.9-1.8 3.7-3.5 5.6-5.3.5-.5 1.4-.5 2-.1l.6.6c1.6 1.6 3.3 3.1 4.9 4.7.6.6.7 1.3.3 1.9-.5.7-1.5.9-2.2.3-.8-.7-1.5-1.4-2.2-2.1-.2-.2-.5-.4-.8-.6.1-.1.1 0 0 0zM323 536h-6.5c-.7 0-1.4-.5-1.5-1.2-.1-.8.4-1.4 1.1-1.6h13.2c.8 0 1.4.4 1.6 1.1.3.9-.4 1.8-1.4 1.8-2.2-.1-4.3-.1-6.5-.1z"/>
    </symbol></svg>


<header class="site-header">
    <div class="grid">
        <a href="/" class="site-header__logo">
             <img src="//invest.MoneyBuddy.com/assets/images/MoneyBuddy.svg" alt="MoneyBuddy" />
        </a>

        <nav class="primary-nav">
            <div class="primary-nav__toggle">
                <svg role="img" class="icon--hamburger"><use xlink:href="#hamburger"></use></svg>
                <span class="primary-nav__toggle-label">Menu</span>
            </div>
            <div class="primary-nav__nav">
                    <ul class="primary-nav__main">
                        <li><a href="whyInvest.jsp">Why invest <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
                        <li><a href="aboutUs.jsp">About us <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
                        <li><a href="SaveTax.jsp">Save Tax <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
                        <li><a href="blog.jsp">Blog <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
                        <li><a href="helpCentre.jsp">Help <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
						<li><a href="/crowdfunding">Crowdfunding <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
                        <li class="primary-nav__live-chat"><a href="javascript:void(0)" data-chat="open"><svg role="img" class="icon--livechat"><use xlink:href="#livechat"></use></svg> Live chat <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
                    </ul>
                    <ul class="account-nav">
                        <li><a href="//invest.MoneyBuddy.com/Dashboard" class="account-nav__sign-in">Sign in <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
                        <li><a href="//invest.MoneyBuddy.com/Register" class="account-nav__join">Join <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
                    </ul>
            </div>
        </nav>
    </div>
</header>

	<!--- MAIN SITE CONTENT -->
	<div id="siteContent" class="site__content">
		<div class="site__scene site__scene--fadein">




<div class="hero carousel">

	<ul class="hero-images">
		<li class="hero-image-1"></li>
		<li class="hero-image-2"></li>
		<li class="hero-image-3"></li>
		<li class="hero-image-4"></li>
		<li class="hero-image-5"></li>
		<li class="hero-image-6"></li>
	</ul>

	<div class="hero__wrapper grid">
		<div class="hero__content">
			<h1 data-heading="h1">Join the new wave of investors</h1>
			<p>Don?t settle for poor savings rates, expect more from your money.</p>
			<p>Let MoneyBuddy invest it for you.</p>

			<ul class="hero__links">
				<li><a href="//invest.MoneyBuddy.com/CreatePlan" data-smooth-state data-button="bright arrow" data-ajax-load="index"><span>Try it out <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></span></a></li>
				<li><a href="javascript:void(0)" class="link-arrow">Find out more <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></li>
			</ul>
		</div>
	</div>
	<span class="hero__content__disclaimer">
		<div class="grid">
<span class="risk-warning-badge">
    <svg class="icon--info" role="img"><use xlink:href="#info"></use></svg>
    With investing your capital is at risk
</span>

		</div>
	</span>
	<div class="hero__down-arrow bounce">
		<svg role="img" class="icon--arrow-dropdown"><use xlink:href="#arrow-dropdown"></use></svg>
	</div>
</div>


		<main class="site">

<div class="global-returns">
    <div class="grid">
        <div class="global-returns__content">
            <p class="global-returns--heading anim1">In the past five years, global stock markets returned an average of 8.89% per annum.</p>

            <table class="global-returns--table anim2">
                <thead>
                    <tr>
                        <th colspan="5">
                            MSCI World Net Index (GBP)
                            <a class="tooltip icon--circle icon--white" title="The MSCI World is a stock market index of 1,643 global stocks. It is maintained by MSCI Inc. (formerly Morgan Stanley Capital International) and is widely used as a common benchmark for global stock funds."><svg role="img" class="icon--info"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#info"></use></svg></a>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <th>2011</th>
                        <th>2012</th>
                        <th>2013</th>
                        <th>2014</th>
                        <th>2015</th>
                    </tr>
                    <tr>
                        <td>-4.9%</td>
                        <td>10.7%</td>
                        <td>24.3%</td>
                        <td>11.5%</td>
                        <td>4.9%</td>
                    </tr>
                </tbody>
            </table>

			<p class="global-returns--heading anim3">Don't miss out - be part of it with MoneyBuddy.</p>



			<span class="anim4"><a href="http://www.MoneyBuddy.com/why-invest" class="global-returns__link" data-button="mid arrow">More reasons to invest<svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a></span>

			<p class="global-returns--small-print anim4">
                <span class="risk-warning-badge anim4">
                    <svg class="icon--info" role="img"><use xlink:href="#info"></use></svg>
                    Past performance is not a guarantee of future return
                </span>
            </p>
        </div>

        <div class="global-returns__canvas">
            <canvas id="globe-canvas" width="1232" height="1200" class="global-returns__canvas--animation"></canvas>
        </div>
    </div>
</div>
<div class="effortless-investing">
    <div class="grid">
        <h3 data-heading="h3 center" class="effortless-investing__heading">Effortless investing</h3>

        <div class="effortless-investing--item">
            <div class="effortless-investing--image ei-diamond--1">
                <h3 data-heading="h3" class="effortless-investing--image__title">
                    <span class="ei-number">1</span><br>
                    <span class="ei-text">You choose</span>
                </h3>
            </div>
            <div class="effortless-investing--text">
                <h3 data-heading="h3" class="effortless-investing--text__title">
                    You choose
                </h3>
                <p>Tell us what type of investor you want to be: cautious, adventurous or somewhere in-between.</p>
            </div>
        </div>
        <div class="effortless-investing--item">
            <div class="effortless-investing--image ei-diamond--2">
                <h3 data-heading="h3" class="effortless-investing--image__title">
                    <span class="ei-number">2</span><br>
                    <span class="ei-text">We invest</span>
                </h3>
            </div>
            <div class="effortless-investing--text">
                <h3 data-heading="h3" class="effortless-investing--text__title">
                    We invest
                </h3>
                <p>We build you a Personal Investment Plan with just the right mix of investments.</p>
            </div>
        </div>
        <div class="effortless-investing--item">
            <div class="effortless-investing--image ei-diamond--3">
                <h3 data-heading="h3" class="effortless-investing--image__title">
                    <span class="ei-number">3</span><br>
                    <span class="ei-text">We optimise</span>
                </h3>
            </div>

            <div class="effortless-investing--text">
                <h3 data-heading="h3" class="effortless-investing--text__title">
                    We optimise
                </h3>
                <p>We monitor your Plan 365 days a year and adjust it to keep everything on track.</p>
            </div>
        </div>
    </div>
</div>

	<div class="wfe carousel">

    <div class="wfe__prev">
        <button type="button" data-role="none" class="slick-prev" aria-label="previous"><svg role="img" class="icon--arrow-back"><use xlink:href="#arrow-back"></use></svg></button>
    </div>

    <div class="wfe__next">
        <button type="button" data-role="none" class="slick-next" aria-label="next"><svg role="img" class="icon--arrow"><use xlink:href="#arrow"></use></svg></button>
    </div>

    <div class="grid">

        <h3 data-heading="h1 center" class="wfe__heading">INVESTING REIMAGINED</h3>

        <ul class="wfe__nav">
            <li>Low fees</li>
            <li>Withdraw anytime</li>
            <li>Access 24/7</li>
			<li>Low minimums</li>
        </ul>

        <div class="wfe__dots"></div>

        <div class="wfe__items">

            <div class="wfe-item wfe-item--1">
                <div class="wfe-item__image">
                    <img srcset="//invest.MoneyBuddy.com/assets/images/imac@2x.png 2x" src="//invest.MoneyBuddy.com/assets/images/imac.png" alt="">
                </div>
                <div class="wfe-item__content">
                    <h4 class="wfe-item__heading">Investing is now affordable.</h4>
                    <p>One simple fee with no hidden charges. Starting from 0.7% a year, going down to as little as 0.5% for larger investments.</p>

                    <a href="http://www.MoneyBuddy.com/why-invest/fees" class="wfe-item__link">About our fees <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a>
                </div>
            </div>
            <!-- end of wfe__item -->

            <div class="wfe-item wfe-item--2">
                <div class="wfe-item__image">
					<img srcset="//invest.MoneyBuddy.com/assets/images/ipad@2x.png 2x" src="//invest.MoneyBuddy.com/assets/images/ipad.png" alt="">
                </div>
                <div class="wfe-item__content">
                    <h4 class="wfe-item__heading">Your money is not locked in.</h4>
                    <p>Add to your Plan or withdraw your money whenever you like, without notice or penalty.</p>



                    <a href="http://www.MoneyBuddy.com/why-invest/how-it-works" class="wfe-item__link">How MoneyBuddy works <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a>
                </div>
            </div>
            <!-- end of wfe__item -->

            <div class="wfe-item wfe-item--3">
                <div class="wfe-item__image">
					<img srcset="//invest.MoneyBuddy.com/assets/images/macbook@2x.png 2x" src="//invest.MoneyBuddy.com/assets/images/macbook.png" alt="">
                </div>
                <div class="wfe-item__content">
                    <h4 class="wfe-item__heading">You're in control, day and night.</h4>
                    <p>Check how your Plan is performing 365 days a year, online or via mobile. Edit your Plan details, or start a new one at any time.</p>




					<a href="http://www.MoneyBuddy.com/why-invest/how-it-works" class="wfe-item__link">How MoneyBuddy works <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a>
                </div>
            </div>
            <!-- end of wfe__item -->

			<div class="wfe-item wfe-item--4">
                <div class="wfe-item__image">
					<img srcset="//invest.MoneyBuddy.com/assets/images/phone@2x.png 2x" src="//invest.MoneyBuddy.com/assets/images/phone.png" alt="">
                </div>
                <div class="wfe-item__content">
                    <h4 class="wfe-item__heading">MoneyBuddy is for everyone.</h4>
                    <p>Whether you're starting with &pound;250 or &pound;2,000,000, MoneyBuddy is for you. And you can add from &pound;20 to your investment each month too, if you like.</p>


                    <a href="//invest.MoneyBuddy.com/CreatePlan" class="wfe-item__link">Try it out <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a>
                </div>
            </div>
            <!-- end of wfe__item -->



        </div>
        <!-- end of wfe__items -->
<span class="risk-warning-badge risk-warning-badge-block">
    <svg class="icon--info" role="img"><use xlink:href="#info"></use></svg>
    With investing your capital is at risk
</span>

    </div>
</div>
<div class="in-safe-hands">
    <div class="grid">
        <h3 data-heading="h1 center" class="in-safe-hands__heading">You're in safe hands</h3>
        <div class="in-safe-hands__wrapper">
            <div class="in-safe-hands--trust-block">
                <div class="in-safe-hands--trust-block__contents">
                    <img src="//invest.MoneyBuddy.com/assets/images/MoneyBuddy--positive.svg" alt="MoneyBuddy" />
                    <div class="in-safe-hands--trust-block__click">
                        <svg class="icon--add icon--white" role="img"><use xlink:href="#add"></use></svg>
                    </div>
                    <div class="in-safe-hands--trust-block__inner">
                        <span>Authorised and regulated by the FCA, we are 100% independent and our investment team has over 55 years of combined industry experience.

								<br><a href="http://www.MoneyBuddy.com/about-us" class="link-arrow">Meet the team <svg role="img" class="icon--arrow-small"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#arrow-small"></use></svg></a>



						</span>
                    </div>
                </div>
            </div>
            <div class="in-safe-hands--trust-block">
                <div class="in-safe-hands--trust-block__contents">
                    <img src="//invest.MoneyBuddy.com/assets/images/partner-logos/Winterflood.gif" class="wf-logo" />
                    <div class="in-safe-hands--trust-block__click">
                        <svg class="icon--add icon--white" role="img"><use xlink:href="#add"></use></svg>
                    </div>
                    <div class="in-safe-hands--trust-block__inner">
                        <span>Your investments and cash are held in custody by Winterflood Business Services. They are part of Close Brothers Group, founded in 1878, who look after billions of assets worldwide.



						</span>
                    </div>
                </div>
            </div>
            <div class="in-safe-hands--trust-block">
                <div class="in-safe-hands--trust-block__contents">
                    <img src="//invest.MoneyBuddy.com/assets/images/partner-logos/fscs.png" />
                    <div class="in-safe-hands--trust-block__click">
                        <svg class="icon--add icon--white" role="img"><use xlink:href="#add"></use></svg>
                    </div>
                    <div class="in-safe-hands--trust-block__inner">
						<span>Up to the first &pound;50,000 of your money can be protected by the Financial Services Compensation Scheme
									<br><a href="/faq/legal-regulatory#legal-covered-by-fscs" class="link-arrow">Find out more <svg role="img" class="icon--arrow-small"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#arrow-small"></use></svg></a>


						</span>
                    </div>
                </div>
            </div>
        </div>
        <p class="in-safe-hands__text">We use funds from leading providers to build your Plan</p>
    </div>
</div>


<div class="fund-providers carousel">
    <div class="grid">
        <div class="fund-providers--edge-fade__left"><img src="//invest.MoneyBuddy.com/assets/images/carouselle-fade-left.png" /></div>
        <div class="company-logos">
            <div><img src="//invest.MoneyBuddy.com/assets/images/fund-providers/Henderson.gif" /></div>
            <div><img src="//invest.MoneyBuddy.com/assets/images/fund-providers/Aberdeen.gif" /></div>
            <div><img src="//invest.MoneyBuddy.com/assets/images/fund-providers/Legal-and-general.gif" /></div>
            <div><img src="//invest.MoneyBuddy.com/assets/images/fund-providers/Vanguard.gif" /></div>
            <div><img src="//invest.MoneyBuddy.com/assets/images/fund-providers/Fidelity.gif" /></div>
            <div><img src="//invest.MoneyBuddy.com/assets/images/fund-providers/Blackrock.gif" /></div>
            <div><img src="//invest.MoneyBuddy.com/assets/images/fund-providers/iShares.gif" /></div>
        </div>
        <div class="fund-providers--edge-fade__right"><img src="//invest.MoneyBuddy.com/assets/images/carouselle-fade-right.png" /></div>
    </div>
</div>


<div class="mic">
    <div class="grid">
        <h3 data-heading="h1 center" class="mic__heading">Made in Cardiff</h3>
        <p class="mic__text">We are based in the fastest-growing capital city in Europe, a hotspot for entrepreneurial, financial services and technology companies.</p>

						<a href="http://www.MoneyBuddy.com/about-us" data-button="light arrow">Meet the team <svg role="img" class="icon--arrow-small"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#arrow-small"></use></svg></a>



    </div>
</div>

<div class="circles-promo">
    <div class="grid">
        <div class="circles-promo__content">
            <h3 class="circles-promo__heading">Grow your Circle,<br />Shrink your fees.</h3>
            <p>Once you've signed up, you'll be able to invite people to join your MoneyBuddy Circle. And the more people you have in your Circle, the bigger the discount you'll get on your monthly fee.</p>


			<a href="http://www.MoneyBuddy.com/why-invest/circles" class="circles-promo__link link-arrow">More about circles <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></a>
        </div>

        <div class="circles-promo__canvas">
            <canvas id="circles-canvas" width="900" height="900" class="circles__canvas"></canvas>
        </div>
    </div>
</div>

<div class="cta-stripe">
    <div class="grid">
        <div class="cta-stripe__content">
            <h3 data-heading="h3" class="cta-stripe__tagline">Expect more from your money</h3>
            <p>MoneyBuddy is the new way to invest.</p>
            <div class="cta-stripe__cta">
                <a href="//invest.MoneyBuddy.com/CreatePlan" data-button="light arrow"><span>Try it out <svg role="img" class="icon--arrow-small"><use xlink:href="#arrow-small"></use></svg></span></a>
            </div>
        </div>
    </div>
</div>

		</main>





		</div>
    </div>
	<!-- /SITE CONTENT -->

	<div id="outdated"></div>



<footer class="site-footer">
    <div class="grid">

        <ul class="social-nav">
            <li>
				<a href="https://www.facebook.com/MoneyBuddy-734189690028131" target="_blank" class="social-nav__facebook">
                	<div class="social-icon-wrapper"><svg role="img" class="icon--facebook"><use xlink:href="#facebook"></use></svg></div>
            	</a>
			</li>
            <li>
				<a href="https://twitter.com/MoneyBuddy_com" target="_blank" class="social-nav__twitter">
                	<div class="social-icon-wrapper"><svg role="img" class="icon--twitter"><use xlink:href="#twitter"></use></svg></div>
            	</a>
			</li>
            <li>
				<a href="https://www.linkedin.com/company/MoneyBuddy" target="_blank" class="social-nav__linkedin">
                	<div class="social-icon-wrapper"><svg role="img" class="icon--linkedin"><use xlink:href="#linkedin"></use></svg></div>
           		</a>
			</li>
        </ul>

        <ul class="footer-nav">
            <li><a href="//invest.MoneyBuddy.com/Register/ClientTerms">Terms &amp; Conditions</a></li>
            <li><a href="/privacy">Privacy policy</a></li>
            <li><a href="/faq">FAQs</a></li>
			<li><a href="/help-centre">Help centre</a></li>
			<li><a href="/why-invest/fees">Our fees</a></li>
        </ul>

        <div data-row>
            <div class="footer__wealth-warning">Warning: Please remember the value of your investments can go down as well as up, so you might get back less than you originally invested.</div>
			<div class="footer__status-disclosure">MoneyBuddy Limited is authorised and regulated by the Financial Conduct Authority (<a href="http://www.fca.org.uk/register" target="_blank">FCA No. 662530</a>) registered in England and Wales (Company No. 09034828) with a registered office at<br>
				<address class="footer__address">Global Reach, Dunleavy Drive, Cardiff, CF11 0SN</address>
					<div class="footer__telephone">Tel: 0800 802 1800</div>
			</div>

            <div class="footer__copyright">&copy; 2016 MoneyBuddy Limited</div>
			<p class="press-number">Press Enquiries: 02920 099073</p>
        </div>
    </div>
</footer>






                <script>
                    window.assetsPath = '//invest.MoneyBuddy.com/assets/';
                    window.appPath = 'https://invest.MoneyBuddy.com/';
                </script>


	<script src="https://cdnjs.cloudflare.com/ajax/libs/modernizr/2.8.3/modernizr.min.js"></script>
    <script>window.Modernizr || document.write('<script src="//invest.MoneyBuddy.com/assets/js/vendor/modernizr.min.js"><\/script>');</script>
    <!--[if lt IE 9]>
		<link rel="stylesheet" type="text/css"  href="//invest.MoneyBuddy.com/assets/vendor/outdatedbrowser/outdatedbrowser.min.css" />
		<link rel="stylesheet" type="text/css"  href="//invest.MoneyBuddy.com/assets/css/ie.css" />
        <script src="//invest.MoneyBuddy.com/assets/js/vendor/html5shiv.min.js"></script>
    <![endif]-->

    <script src="//ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js"></script>
    <script>window.jQuery || document.write('<script src="//invest.MoneyBuddy.com/assets/js/vendor/jquery-1.11.2.js"><\/script>')</script>
    <script src="//invest.MoneyBuddy.com/assets/js/vendor/jquery.validate.min.js"></script>
    <script src="//invest.MoneyBuddy.com/assets/js/vendor/jquery.validate.unobtrusive.min.js"></script>
	<script src="//invest.MoneyBuddy.com/assets/vendor/outdatedbrowser/outdatedbrowser.min.js"></script>
    <script src="//invest.MoneyBuddy.com/assets/js/vendor/jquery.smoothState.min.js"></script>
	<script src="//invest.MoneyBuddy.com/assets/js/scripts/create-js.min.js"></script>
	<script src="//invest.MoneyBuddy.com/assets/js/scripts.min.js?v=2235235"></script>






</body>
</html>