...
public class QueryStpCart {
	...
	Logger logger = Logger.getLogger(QueryStpCart.class);
		
	public List<StpCart> getCustomerStpCart(String customerId) throws MoneyBuddyException {
		
		logger.debug("QueryCustomerRedemptionCart class - getCustomerStpCart method - customerId - "+customerId+" - start");
		...
		try
		{
			...
			logger.debug("QueryCustomerRedemptionCart class - getCustomerStpCart method - customerId - "+customerId+" - returns stpCartList of - "+(stpCartList.size()-1)+" records");
			logger.debug("QueryCustomerRedemptionCart class - getCustomerStpCart method - customerId - "+customerId+" - end");
			
			return stpCartList;
		}
		catch ( HibernateException e ) {
			logger.error("QueryCustomerRedemptionCart class - getCustomerStpCart method - customerId - "+customerId+" - Caught HibernateException");
			e.printStackTrace();
			throw new MoneyBuddyException(e.getMessage(),e);
		}
		catch (Exception e ) {
			logger.error("QueryCustomerRedemptionCart class - getCustomerStpCart method - customerId - "+customerId+" - Caught Exception");
			e.printStackTrace();
			throw new MoneyBuddyException(e.getMessage(),e);
		}
		finally {
